...
CDSoundIn::CDSoundIn(void)
{
	...
	{
		QMessageBox::warning(NULL, "warning", "CDSoundIn CoInitialize Failed!\r\n");
		exit(0);
	}
	m_pCapDev = NULL;
	m_pCapBuf = NULL;
...
void CDSoundIn::notify_capture_thd(LPVOID data)
{
	...
	while (pado->m_bRecording) {
		...
		case WAIT_OBJECT_0 + 0:
			...
			if (FAILED(hr = pado->RecordCapturedData())) {
				QMessageBox::warning(NULL, "warning", "Error handling DirectSound notifications.");
...
HRESULT CDSoundIn::StartOrStopRecord(BOOL bStartRec)
{
	...
	if (bStartRec) {
		...
		_beginthread(CDSoundIn::notify_capture_thd, 0, (void*)(this));
	}
	else {
